...
import {ActivityIndicator, Dimensions, FlatList, Image, PixelRatio, Platform, ScrollView, StyleSheet, Text, TextInput, TouchableHighlight, View} from "react-native";
import Qs from "qs";
...
export default class GooglePlacesAutocomplete extends Component {
  ...
  _request = (text) => {
    ...
    if (text.length >= this.props.minLength) {
      ...
      request.open('GET', 'https://maps.googleapis.com/maps/api/place/autocomplete/json?&input=' + encodeURIComponent(text) + '&' + Qs.stringify(this.props.query) + '&components=country:lk');
      if (this.props.query.origin !== null) {
        request.setRequestHeader('Referer', this.props.query.origin)
      }
  ...
  _renderRowData = (rowData) => {
    ...
    return (
        <AppText style={[{flex: 1}, defaultStyles.description, this.props.styles.description, rowData.isPredefinedPlace ? this.props.styles.predefinedPlacesDescription : {}]}
              numberOfLines={1}
        >
    ...
    if (this.props.renderRightButton) {
      ...
      return (
        <Button style={{alignItems: 'center', justifyContent: 'center'}}
          ...
          this.setState({text: ''})
        }}><IconFa style={{fontSize: 18, marginRight: 10, alignSelf: 'center'}} name={'times-circle'}/></Button>)
    }
  };
  ...
  render() {
    ...
    let containerHeight = this.state.listViewDisplayed === true && this.state.dataSource.length>0  ? 300 : 45;
    return (
        <View
            style={[defaultStyles.container, this.props.styles.container, {height: containerHeight}]}
