...
for fileToModify in $filesToModify; do
  sed -i.bak "s/BringMe/${newAppName}/g;s/pepperoniapptemplate/${newLowerCaseName}/g" $fileToModify
done
...
for fileToRename in $filesToRename; do
  newName=$(echo $fileToRename | sed "s/BringMe/$newAppName/g;s/pepperoniapptemplate/$newLowerCaseName/g")
